  ...
  const handleView = (item) => {
    ...
    setModalContent(
      <div style={{ display: "flex", flexDirection: "column", gap: 16, paddingBottom: 16 }}>
        <FaTimes onClick={handleClose} style={{ position: "sticky", top: 0, alignSelf: "flex-end", cursor: "pointer", fontSize: 20, color: "#333" }} />
        <h3 style={{ background: "linear-gradient(135deg, #00b874, #00d2ff)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", fontWeight: "bold", margin: 0 }}>
          Appointment Details
        </h3>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: 12, borderRadius: 12, boxShadow: "0 4px 15px rgba(0,0,0,0.08)", background: "#fff" }}>
          {item.client?.photo ? (
            <img src={item.client.photo} alt={item.client?.name || "Client"} style={{ width: 60, height: 60, borderRadius: "50%", objectFit: "cover" }} />
          ) : (
            <div style={{ width: 60, height: 60, borderRadius: "50%", background: "#ccc", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, color: "#fff" }}>
              <FaUser />
            </div>
          )}
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <h5 style={{ margin: 0 }}>{item.client?.name || "Unknown Client"}</h5>
            {item.client?.phone && <p style={{ margin: 0, display: "flex", alignItems: "center", gap: 6 }}><FaPhone /> {item.client.phone}</p>}
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: 12, borderRadius: 12, boxShadow: "0 4px 15px rgba(0,0,0,0.08)", background: "#fff" }}>
          <p style={{ margin: 0, display: "flex", alignItems: "center", gap: 6 }}><FaClock /> Booking: {formatDateTime(item.booking_date)}</p>
          <p style={{ margin: 0, display: "flex", alignItems: "center", gap: 6 }}><FaNotesMedical /> Service: {item.service} / {item.format}</p>
          <p style={{ margin: 0, display: "flex", alignItems: "center", gap: 6 }}><FaUser /> Fees: ₹{item.transaction?.amount || "-"}</p>
          <p style={{ margin: 0, display: "flex", alignItems: "center", gap: 6 }}><FaUser /> Payment Status: <span style={{ color: getPaymentStatusColor(item.transaction?.status?.name), fontWeight: 600 }}>{item.transaction?.status?.name || "-"}</span></p>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, padding: 12, borderRadius: 12, boxShadow: "0 4px 15px rgba(0,0,0,0.08)", background: "#fff" }}>
          <p style={{ margin: 0, display: "flex", alignItems: "center", gap: 6 }}><FaUser /> Booked For: {item.whom === "Self" ? "Self" : item.cname}</p>
          {item.whom !== "Self" && <>
            <p style={{ margin: 0, display: "flex", alignItems: "center", gap: 6 }}><FaUser /> Relation: {item.relation_with_client}</p>
            <p style={{ margin: 0, display: "flex", alignItems: "center", gap: 6 }}><FaUser /> Age: {item.age}</p>
          </>}
        </div>

        {/* Notes */}
        <div style={{ display: "flex", flexDirection: "column", gap: 6, padding: 12, borderRadius: 12, boxShadow: "0 4px 15px rgba(0,0,0,0.08)", background: "#fff" }}>
          <p style={{ margin: 0, fontWeight: "bold", display: "flex", alignItems: "center", gap: 6 }}><FaNotesMedical /> Notes</p>
