import React from "react";
import "./appointment-tab-content.css";
import ModalComponent from "../Modal/Modal-component";
import { defaultProfile, imagePath } from "../../../utils/url";
import { formatDate, formatDateTime } from "../../../utils/time";

const AppointmentsContent = ({appointments}) => {
  const [open, setOpen] = React.useState(false);
  const [modalContent, setModalContent] = React.useState(null);

  const handleOpen = (content) => {
    setModalContent(content);
    setOpen(true);
  };

  console.log("appointments",appointments);

  const handleClose = () => setOpen(false);

  return (
    <>
      <div
        className="tab-pane fade show active"
        id="pills-upcoming"
        role="tabpanel"
        aria-labelledby="pills-upcoming-tab"
      >
        {appointments && appointments.map((appointment, index) => (
          <div className="appointment-wrap" key={index}>
            <ul>
              <li>
                <div className="patinet-information">
                  
                  <div className="patient-info">
                    <p style={{fontSize:"14px",color:"#131313ff"}}><i className="fa-solid fa-user"></i> {appointment.client?.name}{appointment.client.age &&` (${appointment.client.age}Y)`}</p>
                    <h6 style={{marginTop:"-3px"}}>
                    <p style={{fontSize:"14px",color:"#646464ff"}}><i className="fa-solid fa-envelope"></i> {appointment.client?.email}</p>
                    </h6>
                    <h6 style={{marginTop:"-3px"}}>
                    <p style={{fontSize:"14px",color:"#646464ff"}}><i className="fa-solid fa-phone"></i> {appointment.client?.phone}</p>
                    </h6>
                  </div>
                </div>
              </li>
              <li className="appointment-info">
                <p>
                  <i className="fa-solid fa-clock"></i> {formatDateTime(appointment.booking_date)}
                  <ul className="d-flex apponitment-types">
                     <li>{appointment.service}</li>
                      <li>{appointment.format}</li>
                  </ul>
                </p>
              </li>
              <li className="mail-info-patient">
                {appointment.whom==="Self"?"Self":`Name: ${appointment.cname}`}
                <br />
                 {appointment.whom!=="Self"&&`Relation: ${appointment.relation_with_client}`}
                 <br />
                 {appointment.whom!=="Self"&&`Age: ${appointment.age}`}
              </li>
              <li className="appointment-start">
                {appointment.status==="New"?<span class="rbt-badge-5 bg-color-warning-opacity color-warning">New</span>:appointment.status==="Completed"?
               <span class="rbt-badge-5 bg-color-success-opacity color-success">Completed</span>:<span class="rbt-badge-5 bg-color-danger-opacity color-danger">Canceled</span>}
                
              </li>
              <li className="appointment-start">
                <button
                  className="start-link"
                  onClick={() =>
                    handleOpen(
                      <div className="appointment-wrap appointment-detail-card">
                        <ul>
                          <li>
                            <div className="patinet-information">
                              <a href="#">
                                <img
                                  className="modal-img"
                                  src="data:image/jpeg;base64,"
                                  alt="User Image"
                                />
                              </a>
                              <div className="patient-info">
                                <p>#Apt0001</p>
                                <h6>
                                  <a href="#">Kelly Joseph </a>
                                </h6>
                                <div className="mail-info-patient">
                                  <ul>
                                    <li>
                                      <i className="fa-solid fa-envelope"></i>
                                      kelly@example.com
                                    </li>
                                    <li>
                                      <i className="fa-solid fa-phone"></i>+1 504
                                      368 6874
                                    </li>
                                  </ul>
                                </div>
                              </div>
                            </div>
                          </li>
                          <li className="appointment-info">
                            <div className="person-info">
                              <p>Type of Appointment</p>
                              <ul className="d-flex apponitment-types">
                                <li>
                                  <i className="fa-solid fa-hospital text-green"></i>
                                  Direct Visit
                                </li>
                              </ul>
                            </div>
                          </li>
                          <li className="appointment-action">
                            <div className="detail-badge-info">
                              <span className="badge-modal">New Patient</span>
                              <span className="badge-modal">Upcoming</span>
                            </div>
                            <div className="consult-fees">
                              <h6>Consultation Fees : $200</h6>
                            </div>
                          </li>
                        </ul>
                        <div className="modal-line"></div>
                        <ul className="detail-card-bottom-info">
                          <li>
                            <h6>Appointment Date &amp; Time</h6>
                            <span>22 Jul 2023 - 12:00 pm</span>
                          </li>
                          <li>
                            <h6>Clinic Location</h6>
                            <span>Adrian’s Dentistry</span>
                          </li>
                          <li>
                            <h6>Location</h6>
                            <span>Newyork, United States</span>
                          </li>
                          <li>
                            <h6>Visit Type</h6>
                            <span>General</span>
                          </li>
                          <li>
                            <div className="start-btn">
                              <button className="btn">Start Session</button>
                            </div>
                          </li>
                        </ul>
                      </div>
                    )
                  }
                >
                  View
                </button>&nbsp;&nbsp;
                <button
                  className="start-link"
                  onClick={() =>
                    handleOpen(
                      <div className="appointment-wrap appointment-detail-card">
                        skd
                      </div>
                    )
                  }
                >
                  Start
                </button>
              </li>
            </ul>
          </div>
        ))}

        <div className="col-lg-12" style={{ marginTop: "20px" }}>
          <nav>
            <ul className="rbt-pagination justify-content-center">
              <li>
                <a aria-label="Previous" href="/elements/style-guide#">
                  <i className="feather-chevron-left"></i>
                </a>
              </li>
              <li>
                <a href="/elements/style-guide#">1</a>
              </li>
              <li className="active">
                <a href="/elements/style-guide#">2</a>
              </li>
              <li>
                <a href="/elements/style-guide#">3</a>
              </li>
              <li>
                <a aria-label="Next" href="/elements/style-guide#">
                  <i className="feather-chevron-right"></i>
                </a>
              </li>
            </ul>
          </nav>
        </div>
      </div>
      <ModalComponent
        open={open}
        handleClose={handleClose}
        content={modalContent}
      />
    </>
  );
};

export default AppointmentsContent;
